(function(){
  ...
  async function createTop100Table() {
    console.log('Starting Top 100 Freshmen script...');
    ...
    try {
      ...
      showLoadingIndicator('Loading roster data...');
      ...
      console.log('Found ' + players.length + ' freshmen, showing top 100');
  ...
  function displayTable(players, lastUpdated) {
    ...
    container.id = 'top-100-freshmen-table';
    container.style.cssText = 'width: 100%; max-width: 1200px; margin: 0 auto 40px auto; padding: 20px; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); position: relative; z-index: 1;';
    ...
    players.forEach(function(player, index) {
      ...
      [
        ...
        td.textContent = text;
        td.style.cssText = 'padding: 10px 8px; background: inherit;' + (i === 0 ? ' font-weight: bold; text-align: center;' : '');
        row.appendChild(td);
      });
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      setTimeout(createTop100Table, 500);
    });
  } else {
    setTimeout(createTop100Table, 500);
  }
